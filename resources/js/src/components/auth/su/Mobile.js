...
export const Mobile = () => {
    ...
    const submitHandler = async () => {
        ...
        if (pace === 1) {
            if (inputs.account !== '') {
                let rqx1 = await request('GET', `/api/auth/su/attempt/mobile`, '', {...inputs, pace: pace})
                if (rqx1.msg === 'success') {
                    let rqx2 = await request('GET', `https://mg8zteiueb.execute-api.ap-southeast-1.amazonaws.com/Development/reset/pw/mobile_num`, '', {...inputs, pace: 'ext'})
                    if (rqx2.msg === 'delivered') {
                        setVerification(rqx2.vc)
                        setPace(2)
                        setDisable(false)
                    }
                } else {
            ...
            if (inputs.password !== '') {
                let rqx = await request('GET', `/api/auth/su/attempt/mobile`, '', {...inputs, pace: pace})
                console.log(rqx);
                if (rqx.msg === 'success') {
                    ...
                    <Box>
                        <Box display={pace===1?'block':'none'}>
                            <Box style={validation.email !== '' ? {border: '1px solid red'} : {}} className={classes.search} mb={2}>
                                ...
                                }} inputProps={{ 'aria-label': 'Mobile Number' }} onChange={(e)=>setInputs({...inputs, account: e.target.value})} style={{paddingLeft: '22px'}} />
                            </Box>
